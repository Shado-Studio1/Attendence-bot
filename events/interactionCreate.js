  ...
  async execute(interaction) {
    ...
    switch (interaction.customId) {
      ...
      case "logout":
        ...
        // Save session data
        if (!sessionsData[interaction.user.id]) {
          sessionsData[interaction.user.id] = {
            username: interaction.user.displayName,
            totalTime: 0,
            sessions: 0
          };
        }
        sessionsData[interaction.user.id].totalTime += duration;
        sessionsData[interaction.user.id].sessions += 1;
        sessionsData[interaction.user.id].username = interaction.user.displayName;
        saveSessions();
